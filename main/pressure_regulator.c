...
void regulator_init(PressureRegulator* reg) {
    ...
    // rate_integral_max ВЫЧИСЛЯЕТСЯ НИЖЕ — после того как присвоены valve_max и
    // valve_flow_floor (см. там). Если посчитать здесь, они ещё нули после memset,
    // и кап выходит (0-0)/ki = 0 -> интегратор заклампен в ноль, контур работает
    // как чистый П и не открывает иглу шире floor+Kp*err (ровно тот баг в логах).
    ...
    // домотать иглу вплоть до valve_max, а настоящий анти-windup (условная
    // интеграция в rate_control_step — заморозка, когда выход упёрся в valve_max)
    // включается ровно на упоре. Меньший кап искусственно ограничил бы открытие
    // иглы (как старое 200: пиннил pos~4210 и не пускал в зону сильного потока).
    // СЧИТАЕМ ЗДЕСЬ, а не выше: нужны уже присвоенные valve_max/valve_flow_floor/rate_ki.
    reg->rate_integral_max = (reg->valve_max - reg->valve_flow_floor) / reg->rate_ki; // ≈ (10000-30)/20 = 498
    ...
    reg->hold_lead_close_s = 2.8f;      // упреждение входа в HOLD (тормозной путь = |rate|*это). По логам
